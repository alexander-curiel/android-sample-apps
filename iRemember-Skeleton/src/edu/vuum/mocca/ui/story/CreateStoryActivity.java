...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		Intent soundRecordIntent = new Intent(this, SoundRecordActivity.class);
		
		// TODO - Use getOutputMediaFile() to create a new 
		// filename for this specific sound file
		String fileUri = (String) getOutputMediaFile(MEDIA_TYPE_AUDIO).getPath();

		// TODO - Add the filename to the Intent as an extra. Use the
		// Intent-extra name from the SoundRecordActivity class, EXTRA_OUTPUT
		soundRecordIntent.putExtra(SoundRecordActivity.EXTRA_OUTPUT, fileUri);
	...
	private void launchVideoCameraIntent() {
		...
		Uri fileUri = Uri.fromFile(getOutputMediaFile(MEDIA_TYPE_VIDEO));

		// TODO - Add the filename to the Intent as an extra. Use the
		// Intent-extra name from the MediaStore class, EXTRA_OUTPUT
		videoIntent.putExtra(MediaStore.EXTRA_OUTPUT, fileUri);

		// TODO - Specify as an extra that the video quality should be HIGH. Use
		// the Intent-extra name, EXTRA_VIDEO_QUALITY, from the MediaStore class
		// set the video image quality to high
		videoIntent.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 1);

		// TODO - Start a new activity for result, using the new intent and the
		// request code CAMERA_VIDEO_REQUEST
